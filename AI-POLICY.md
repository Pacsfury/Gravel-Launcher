# Gravel's AI Policy

This document outlines the acceptable and unacceptable uses of Artificial Intelligence (AI) for contributions to the Gravel repository.

## Code Contributions
* **Human Oversight Required:** You may use AI tools to assist with coding.
* **Mandatory Testing:** Every line of AI-generated code must be thoroughly tested by a human.
* **Mitigate Risks:** Review all code carefully to prevent AI hallucinations and errors.

## Community & Communication
* **No Automated Replies:** Do not use AI to respond to users in Pull Requests or issues.
* **Human Connection:** All interactions within the community must come from genuine human input.

## Documentation & Issues
* **Full Transparency:** If you use AI to write documentation, commit messages, or issue descriptions, you must disclose it.
* **Clear Attribution:** Specify exactly which parts were generated or assisted by AI. If you use AI to commit, you should write it here, otherwise, I will do it myself:

| Commiter   | Changes                  | Human  |
|------------|--------------------------|--------|
| **Claude** | 107 ++, 82 --, 4 commits | skeeto |
